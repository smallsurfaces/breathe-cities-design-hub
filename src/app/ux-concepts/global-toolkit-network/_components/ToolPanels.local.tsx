/**
 * ToolPanels.local.tsx — Visual sketch components for each of the 8 toolkit capabilities.
 *
 * Purpose
 *   The recognisable visual sketch of each capability (a map, gauge, chart, table, etc.) rendered
 *   with CSS shapes and BC tokens. Used as the PREVIEW inside each catalogue card on the
 *   global-toolkit-network landing. No SVG charting libraries — pure CSS/HTML shapes.
 *
 *   This is a CONCEPT-LOCAL vendored copy for the global-toolkit-network concept (promotion to the
 *   client review hub, 2026-06-29): the concept stands alone with zero dependency on any sibling
 *   `toolkit` concept. Behaviour and all functional colour are identical to the source panels; only
 *   the header and the data-type import path differ (types now come from `./toolkit-data.local`).
 *   The shared aqi-palette (@/components/concept) is consumed read-only.
 *
 ...
 * External dependencies: ./toolkit-data.local (sketch data types, concept-local),
 *   @/components/concept (aqi-palette imports, shared)
 */
